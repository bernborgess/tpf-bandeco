...
float AABBColliderComponent::DetectHorizontalCollision(RigidBodyComponent *rigidBody) {
    ...
    for (auto collider: mOwner->GetGame()->GetColliders()) {
        ...
        // Se o objeto dono do componente estiver colidindo horizontalmente com algum collider,
        //  resolva a colisão horizontal e retorne o valor da interseção horizontal mínima.
        if (!Intersect(*collider)) continue;

        // Utilize o método `GetMinHorizontalOverlap` para calcular a interseção horizontal mínima.
        auto mh = GetMinHorizontalOverlap(collider);
        auto mv = GetMinVerticalOverlap(collider);
        if (Math::Abs(mh) >= Math::Abs(mv)) continue;

        // Utilize o método `ResolveHorizontalCollisions` para resolver a colisão horizontal.
        ResolveHorizontalCollisions(rigidBody, mh);
        // Utilize o método `OnHorizontalCollision` do objeto dono do componente para notificar a colisão ao Actor dono desse componente.
